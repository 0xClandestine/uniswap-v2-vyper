...
def _update(balance0: uint256, balance1: uint256, _reserve0: uint112, _reserve1: uint112):
    ...
    if timeElapsed > 0 and _reserve0 != 0 and _reserve1 != 0:
        self.price0CumulativeLast += unsafe_mul(self.uqdiv(self.encode(_reserve1), _reserve0), convert(timeElapsed, uint256))
        self.price1CumulativeLast += unsafe_mul(self.uqdiv(self.encode(_reserve0), _reserve1), convert(timeElapsed, uint256))
